...
class Utils {
    ...
    public static function bless(Player $player): void
    {
        ...
        foreach ($player->getEffects()->all() as $effect) {
            if ($effect->getType()->isBad()){
                ...
                $removedMsg = str_replace(["{effect}", "{level}"], [$effectName, self::getRomanNumeral($level)], $removedMsg);
    ...
    public static function fillInventory(Inventory $inventory, Item $glassType, array $excludedSlots = []): void
    {
        ...
        for ($slot = 0; $slot < $size; $slot++) {
            if (!in_array($slot, $excludedSlots)) {
                $inventory->setItem($slot, $glassType->setCustomName("§r§8Ethereal Hub"));
            }
        }
    }
    ...
    public static function convertBlockTypeIdToName(int $blockTypeId): string {
        $name = "";

        switch (is_int($blockTypeId)) {
